...
- 第三种 Roadmap carrier（SQLite）与 single-file JSON、bundle 并列，由**同一套 adapter 契约**承载（`Roadmap` 继承 ~40 方法、只覆写存储原语）。
...
- 决策：先做 SQLite，不把事件流升格为主事实源（P4 前提，但单设备阶段只增读放大）；`recommend-storage` 仍只读建议，迁移只走显式命令；bundle 保留为可 diff 导出形态。
