...
- **Local/self-contained:** when one person has full control or works offline, use single-file JSON for ordinary routes; use SQLite when node/decision/history artifacts are large, then render the bounded Markdown view.
...
the fact source; use `migrate --to single|sqlite` after deciding to move.
