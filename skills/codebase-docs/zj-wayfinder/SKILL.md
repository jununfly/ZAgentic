...
- **Tracker mode (teamwork).** The map and its child tickets live on the repo's issue tracker (GitHub/GitLab/…). Wins: multi-agent concurrency via `claim` + native blocking edges, and the human sees the frontier rendered in the tracker's own UI. Needs: a configured tracker — run `/zj-repo-init` if none is provided.
...
**Where the map, its child tickets, blocking, and frontier queries physically live is carrier-specific** — see [Carrier modes](#carrier-modes). In **tracker mode** the issue tracker should have been provided to you (run `/zj-repo-init` if not; consult its "Wayfinding operations" section for how _this_ repo expresses them). In **local mode** — the default when no tracker is configured — everything lives in one local markdown file you create.
