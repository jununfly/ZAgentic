---
name: zj-systematic-research
description: |
  对产品、公司、概念、技术或人物进行系统性研究：重建其从起源到当下的演进脉络，分析当前竞品/同类格局，并综合两条线索形成判断。
  当用户想要系统性了解一个对象的来龙去脉、发展逻辑、竞争位置或整体画像时使用；不要用于单个事实查找、API/文档阅读、局部技术问题或只比较方案的技术选型报告。
---
...
这不是一份冷冰冰的咨询报告，而是一篇让人能从头读到尾的深度研究。写作风格需要在「研究报告的严谨」和「深度可读的表达」之间找到平衡点。
...
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author ""
   ```
